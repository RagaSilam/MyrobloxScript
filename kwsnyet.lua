...
Main.Size = UDim2.new(0, 260, 0, 400); Main.Position = UDim2.new(0.5, -130, 0.5, -200)
...
createBtn("FLING CLOSEST", function()
	local t, d = nil, 80
	...
	if t and not flingActive then
		flingActive = true; local hrp = lp.Character.HumanoidRootPart; local old = hrp.CFrame
		task.spawn(function()
			local s = tick()
			while tick() - s < 0.5 do runService.Heartbeat:Wait()
				if t.Character and t.Character:FindFirstChild("HumanoidRootPart") then 
					hrp.CFrame = t.Character.HumanoidRootPart.CFrame
					hrp.Velocity = Vector3.new(45000, 45000, 45000) 
				end
			end
			flingActive = false; hrp.Velocity = Vector3.new(0,0,0); hrp.CFrame = old
		end)
	end
end)
